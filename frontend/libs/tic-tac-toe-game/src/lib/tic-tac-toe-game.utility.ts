...
export class TicTacToeGameUtility {
  ...
  public resetBoard(): TicTacToeBoardCells {
    ...
    // Create all rows
    for (let boardRowIndex = 0; boardRowIndex < TicTacToeGameUtility.boardHeight; boardRowIndex++) {
      newBoardState.push([]);
      // Create all columns for row
      for (let boardColumnIndex = 0; boardColumnIndex < TicTacToeGameUtility.boardWidth; boardColumnIndex++) {
  ...
  public canCellChange(board: TicTacToeBoardCells, rowIndex: number, columnIndex: number): boolean {
    return board[rowIndex][columnIndex] === TicTacToeCellSate.Empty;
  }

  /** Return cell state for coordinates */
  public getCellState(board: TicTacToeBoardCells, rowIndex: number, columnIndex: number): TicTacToeCellSate {
